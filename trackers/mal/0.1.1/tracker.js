...
// src/mal.ts
var exports_mal = {};
__export(exports_mal, {
  default: () => mal_default
});
module.exports = __toCommonJS(exports_mal);
...
// src/mal.ts
var API_BASE = "https://api.myanimelist.net/v2";
var AUTH_BASE = "https://myanimelist.net/v1/oauth2";
var PER_PAGE = 100;
var MAL_CLIENT_ID = "70ff35a48559cce9655338a1106f7c88";
var SETTINGS = defineSettings([
  {
    ...
    required: true,
    authUrlTemplate: `${AUTH_BASE}/authorize?response_type=code` + "&client_id={clientId}" + "&code_challenge={pkce}" + "&code_challenge_method=plain" + "&redirect_uri={callbackUrl}" + "&state={state}",
    exchange: {
      url: `${AUTH_BASE}/token`,
...
class MalTracker extends TrackerBase {
  ...
  async get(path, params) {
    ...
    if (res.status === 401)
      throw new Error("MAL: invalid or expired access token");
    if (res.status >= 400) {
  ...
  async getLibrary(page) {
    ...
    const data2 = await this.get("/users/@me/mangalist", {
      fields: "list_status,main_picture,num_chapters",
      sort: "list_updated_at",
      limit: String(PER_PAGE),
      offset: String(offset)
    });
    const items = data2.data.map(({ node: node2, list_status }) => {
      const item = {
        externalId: node2.id,
        title: node2.title,
        status: TO_TRACKER[list_status.status] ?? "planning"
      };
      if (list_status.num_chapters_read > 0)
        item.chaptersRead = list_status.num_chapters_read;
      if (node2.num_chapters !== undefined && node2.num_chapters > 0)
        item.totalChapters = node2.num_chapters;
      if (node2.main_picture?.medium)
        item.thumbnailUrl = node2.main_picture.medium;
      return item;
    });
    return { items, page, hasNextPage: !!data2.paging.next };
  }
  async updateEntry(externalId, update2) {
    const fields = {};
    if (update2.status !== undefined)
      fields.status = FROM_TRACKER[update2.status];
    if (update2.chaptersRead !== undefined)
      fields.num_chapters_read = String(Math.floor(update2.chaptersRead));
    if (update2.score !== undefined)
      fields.score = String(Math.round(update2.score / 10));
    if (update2.startedAt !== undefined)
      fields.start_date = update2.startedAt;
    if (update2.finishedAt !== undefined)
      fields.finish_date = update2.finishedAt;
  ...
  async search(query, page) {
    ...
    const data2 = await this.get("/manga", {
      q: query,
      limit: "20",
      offset: String(offset),
      fields: "main_picture,synopsis"
    });
    const items = data2.data.map(({ node: node2 }) => {
      const item = { externalId: node2.id, title: node2.title };
      if (node2.main_picture?.medium)
        item.thumbnailUrl = node2.main_picture.medium;
      if (node2.synopsis)
        item.description = node2.synopsis.slice(0, 300);
      return item;
    });
    return { items, page, hasNextPage: !!data2.paging.next };
  }
...
var mal_default = defineTracker((host) => new MalTracker(host));

//# debugId=4521E49898B039B364756E2164756E21
